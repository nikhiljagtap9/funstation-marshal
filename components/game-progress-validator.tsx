...
export default function GameProgressValidator({
	currentGame,
	games,
	onValidationError,
}: GameProgressValidatorProps) {
		...
		if (currentGame > 0) {
			...
			if (!previousGame?.completed) {
				toast({
					...
					description: `You must complete ${
						previousGame?.name || "the previous game"
					} before proceeding to the next game.`,
					variant: "destructive",
					duration: 5000,
				});
				onValidationError();
